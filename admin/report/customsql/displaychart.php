...
define('REPORT_CUSTOMSQL_MAX_RECORDS', 10);
...
// data for google's chart JS object
$chartdata = array();
...
if (is_null($cvstimestamp)) {
    ...
    list($csvfilename, $cvstimestamp) = report_customsql_csv_filename($report, $cvstimestamp,10);
...
foreach ($chartdata as $title => $data) {
    $postdata .= "['".str_replace('\'','',$title)."',{$data}], ";
}
//$postdata .= '['1',1]';
//$_SESSION['chartdata'] = $postdata ;
echo '<form target="_new" method="post" action="../google-chart-api.php"><textarea name="chartdata" style="display:none;">'.$postdata.'</textarea><input type="submit" value="Chart"></form>';
