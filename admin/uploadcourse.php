...
// Some Regex constants
    ...
    function checkisstring($supposedstring) {
        ...
        return (mb_strlen($supposedstring,'utf8') == 0) ? false : true;
    }
    function validateas($value, $validatename, $lineno, $fieldname = '') {
        ...
        case 1: // String
                if (($maxlen = $format[1])!=0)  // Max length?
                    (mb_strlen($value,'utf8') <=$format[1]) or csverror('Invalid value for field '.$fieldname.' ( "'.$value.'" = length('.mb_strlen($value,'utf8').') &gt; '.$format[1].'). '.
        ...
        case 5: // Category
            if (checkisint($value)) {
              // It's a Category ID Number
               ...
               (mb_strlen($value,'utf8')>0) or csverror('Invalid value for field '.$fieldname.' (Path string not set). '.
                          get_string('erroronline', 'error', $lineno) .". ".
                          get_string('processingstops', 'error'), 
                          'uploadcourse.php?sesskey='.$USER->sesskey);
                ...
                foreach ($cats as $n => $item) { // Validate the path
                  ...
                  (mb_strlen($item,'utf8') <= 30) or csverror('Invalid value for field '.$fieldname.' (Category name "'.$item.'" length &gt; 30). '.
                                      get_string('erroronline', 'error', $lineno) .". ".
                                      get_string('processingstops', 'error'), 
                                      'uploadcourse.php?sesskey='.$USER->sesskey);
        ...
        case 6: // User ID or Name (Search String)
            ...
            if (checkisint($value)) { // User ID
                ...
                if (isset($usersearch) and ($usersearch!==false) and is_array($usersearch) and (($ucount=count($usersearch))>0)) {
                    ...
                    csverror('Invalid value {'.$value.'} for field '.$fieldname.' (Search string returned no results). '.
                                  get_string('erroronline', 'error', $lineno) .". ".
                                  get_string('processingstops', 'error'), 
                                  'uploadcourse.php?sesskey='.$USER->sesskey);
                }
...
/// If a file has been uploaded, then process it
    ...
    ?><center><p>Upload an <a href="http://www.rfc-editor.org/rfc/rfc4180.txt" target="_blank">RFC4180</a>-Compliant CSV file.<br />Valid fields for each course are:</p><table border="1" style="text-align:left;direction:ltr;font-family:monospace;font-size:8pt;width:500px;">
