<?php
  ...
  //$upload_dir = $CFG->dataroot.$_POST["VideoServerFolder"];
  ...
  //$path = make_upload_directory($uploads_dir,false);
	...
	$errfile = "retrive.log";
	//checkong for VideoServerFolder POST variable. Defining VideoLocal variable
	if(isset($_POST["VideoServerFolder"])) {
	    $VideoLocal = $CFG->dataroot.'/'.$_POST["VideoServerFolder"].'/';
	    //$path = make_upload_directory($VideoLocal,false);
  	} else {
	    $VideoLocal="/tmp/";
	}
	//Defining $VideoFolder variable - a folder where Videofiles will be stored
	//$VideoFolder = getcwd().$VideoLocal;
	$VideoFolder = $VideoLocal;
	//setting full path name to log file
	$errfile=$VideoFolder.$errfile;
	...
	{
		AddLogs($errfile,"POST variable received >".$key." = ".$val, "info");
	}
	...
	if(!empty($_POST["ACTION"]))
	{
		...
		{
			//getting userdir variable - user personal directory inside $VideoFolder directory
			//where Videofile will be stored
			if (!empty($_POST["USERDIR"]) ) 
			{
				$UserDir = $VideoFolder . $_POST["USERDIR"] . "/";
		        	...
		        	if (!is_dir($UserDir))
				{
					...
					{   
					        ...
					        if( !mkdir( $UserDir, 0777 ) ) 
						{
					        	...
					        	ErrorMessage("Server cat'n create folder for your Video files...\n Tell about it to System Administrator...");
				        	}
						else 
						{
				...
				Addlogs($errfile, "file name is ".$MVfile_name, "info");
				...
				if( is_uploaded_file($tmp_name) )
				{
					...
					fclose($flocal);
					//writing log message
					Addlogs($errfile, "ACTION: ".$GLOBALS['ACTION']."; exit ACCEPTED", "info");
