...
function ColumnResize(table,editor) {
	...
	this.stopColumnDrag = function(e) {
		...
		editor._doc.onmouseup  = saveOnmouseup;
		editor._doc.onmousemove = saveOnmousemove;
		editor._doc.body.style.cursor = saveBodyCursor;
		...
		editor._doc.cookie = self.id + '-width=' + colWidth +
	...
	this.startColumnDrag = function(e) {
		...
		saveOnmouseup       = editor._doc.onmouseup;
		editor._doc.onmouseup  = self.stopColumnDrag;

		saveBodyCursor             = editor._doc.body.style.cursor;
		editor._doc.body.style.cursor = 'w-resize';

		// fire!
		saveOnmousemove      = editor._doc.onmousemove;
		editor._doc.onmousemove = self.columnDrag;
...
function ResizableColumns(editor) {
    
	var tables = editor._doc.getElementsByTagName('table');
	for (var i=0; tables.item(i); i++) {
		if (tables[i].className.match(/resizable/)) {
            alert ('Start resizing the Table by dragging the headers');
			// generate id
			if (!tables[i].id) tables[i].id = 'table'+(i+1);
			// make table resizable
			resizableTables[resizableTables.length] = new ColumnResize(tables[i],editor);
		}
	}
