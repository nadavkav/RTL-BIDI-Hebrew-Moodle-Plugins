...
//  USER is a persistent variable using sessions
    ...
    function html_header($course, $wdir, $formfield=""){
        ...
        if (!empty($_SERVER['HTTPS']) and $_SERVER['HTTPS'] != 'off') {
            $url = preg_replace('|https?://[^/]+|', '', $CFG->wwwroot).'/lib/editor/htmlarea/custom_plugins/insertpdf/';
        } else {
        ...
        function set_value(params) {
            ...
            try {
                if(insimg != null) {     // mine type image/jpeg image/gif image/png > application/x-shockwave-flash
                    if(params.itype.indexOf("application/pdf") == -1 ) {
                        alert("<?php echo get_string("notpdf","insertpdf",'',$CFG->dirroot.'/lib/editor/htmlarea/custom_plugins/insertpdf/lang/');?>");
                        return false;
                    }
                    for(field in params) {
