...
//  USER is a persistent variable using sessions
    ...
    function html_header($course, $wdir, $formfield=""){
        ...
        if (!empty($_SERVER['HTTPS']) and $_SERVER['HTTPS'] != 'off') {
            $url = preg_replace('|https?://[^/]+|', '', $CFG->wwwroot).'/lib/editor/htmlarea/custom_plugins/hocr/';
        } else {
            $url = $CFG->wwwroot.'/lib/editor/htmlarea/custom_plugins/hocr/';
        }
...
//<![CDATA[
        ...
        function set_value(params) {
            ...
            try {
                if(insimg != null) {     // mine type image/jpeg image/gif image/png > application/x-shockwave-flash
                    if(params.itype.indexOf("image/jpeg") == -1 && params.itype.indexOf("image/png") == -1 && params.itype.indexOf("image/gif") == -1) {
                        alert("<?php echo get_string("notimage","hocr",'',$CFG->dirroot.'/lib/editor/htmlarea/custom_plugins/hocr/lang/');?>");
                        return false;
                    }
                    for(field in params) {
