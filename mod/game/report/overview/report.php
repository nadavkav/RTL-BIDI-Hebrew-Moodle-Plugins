...
//            'qa.uniqueid as attemptuniqueid, qa.id AS attempt, u.id AS userid, u.firstname, u.lastname, u.picture, '.
        ...
        if ($course->id != SITEID) { // this is too complicated, so just do it for each of the four cases.
            if (!empty($currentgroup) && empty($noattempts)) {
                ...
                $from  = 'FROM '.$CFG->prefix.'user u JOIN '.$CFG->prefix.'role_assignments ra ON ra.userid = u.id '.
                    ...
                    'JOIN '.$CFG->prefix.'game_attempts qa ON u.id = qa.userid AND qa.gameid = '.$game->id;
                ...
                $from  = 'FROM '.$CFG->prefix.'user u JOIN '.$CFG->prefix.'role_assignments ra ON ra.userid = u.id '.
                    groups_members_join_sql().
                    'LEFT JOIN '.$CFG->prefix.'game_attempts qa ON u.id = qa.userid AND qa.gameid = '.$game->id;
                ...
                $from  = 'FROM '.$CFG->prefix.'user u JOIN '.$CFG->prefix.'role_assignments ra ON ra.userid = u.id LEFT JOIN '.$CFG->prefix.'game_attempts qa ON u.id = qa.userid AND qa.gameid = '.$game->id;
                ...
                if (empty($noattempts)) {
                    ...
                    $from  = 'FROM '.$CFG->prefix.'user u JOIN '.$CFG->prefix.'game_attempts qa ON u.id = qa.userid ';
        ...
        if ($hasfeedback) {
            ...
            $from .= " JOIN {$CFG->prefix}game_feedback AS qf ON " .
