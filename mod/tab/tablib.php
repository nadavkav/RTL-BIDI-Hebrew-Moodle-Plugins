<?php
   ...
   if ($mod->id) {
    ...
    if ($tab->displayfp == 1 ) {
        ...
        // Display TABs
        echo '<div id="tabcontent_'.$tab->id.'">'."\n";
        echo '<div class=" yui-skin-sam">'."\n";
        echo '<div id="Tabs_'.$tab->id.'" class="yui-navset">'."\n";
        echo '  <ul class="yui-nav">'."\n";
        echo '    <li class="selected"><a href="#tab_'.$tab->id.'_1"><em>'.$tab->tab1.'</em></a></li>'."\n";
        if (!empty($tab->tab2)){
            echo '    <li><a href="#tab_'.$tab->id.'_2"><em>'.$tab->tab2.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab3)){
            echo '    <li><a href="#tab_'.$tab->id.'_3"><em>'.$tab->tab3.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab4)){
            echo '    <li><a href="#tab_'.$tab->id.'_4"><em>'.$tab->tab4.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab5)){
            echo '    <li><a href="#tab_'.$tab->id.'_5"><em>'.$tab->tab5.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab6)){
            echo '    <li><a href="#tab_'.$tab->id.'_6"><em>'.$tab->tab6.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab7)){
            echo '    <li><a href="#tab_'.$tab->id.'_7"><em>'.$tab->tab7.'</em></a></li>'."\n";
        }
        if (!empty($tab->tab8)){
            echo '    <li><a href="#tab_'.$tab->id.'_8"><em>'.$tab->tab8.'</em></a></li>'."\n";
        }
            echo '  </ul>'."\n";
    ...
    $options->noclean = true;
        ...
        echo '     <div id="tab_'.$tab->id.'_1"><p>'.format_text($tab->tab1content, FORMAT_HTML,$options).'</p>'."\n";
        //echo '     <div id="tab1"><p>'.$tab->tab1content.'</p>'."\n";
        echo '</div>'."\n";
        if (!empty($tab->tab2)){
            echo '  <div id="tab_'.$tab->id.'_2"><p>'.format_text($tab->tab2content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab2"><p>'.$tab->tab2content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab3)){
            echo '  <div id="tab_'.$tab->id.'_3"><p>'.format_text($tab->tab3content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab3"><p>'.$tab->tab3content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab4)){
            echo '  <div id="tab_'.$tab->id.'_4"><p>'.format_text($tab->tab4content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab4"><p>'.$tab->tab4content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab5)){
            echo '  <div id="tab_'.$tab->id.'_5"><p>'.format_text($tab->tab5content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab5"><p>'.$tab->tab5content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab6)){
            echo '  <div id="tab_'.$tab->id.'_6"><p>'.format_text($tab->tab6content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab6"><p>'.$tab->tab6content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab7)){
            echo '  <div id="tab_'.$tab->id.'_7"><p>'.format_text($tab->tab7content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab7"><p>'.$tab->tab7content.'</p>'."\n";
            echo '</div>'."\n";
        }
        if (!empty($tab->tab8)){
            echo '  <div id="tab_'.$tab->id.'_8"><p>'.format_text($tab->tab8content, FORMAT_HTML,$options).'</p>'."\n";
            //echo '     <div id="tab8"><p>'.$tab->tab8content.'</p>'."\n";
            echo '</div>'."\n";
        }
        echo '<script type="text/javascript">'."\n";
        echo "    var tabView_$tab->id = new YAHOO.widget.TabView('Tabs_$tab->id');"."\n";
        echo '</script>'."\n";
        echo '  </div>'."\n";
