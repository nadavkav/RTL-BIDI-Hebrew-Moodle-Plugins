<?php // $Id: tabs.php,v 1.11.2.2 2008/06/20 13:36:43 mchurch Exp $
/**
* prints the tabbed bar
*
* @version $Id: tabs.php,v 1.11.2.2 2008/06/20 13:36:43 mchurch Exp $
...
*/
    ...
    if ($questionnaire->capabilities->readownresponses && ($numresp > 0)) {
        ...
        if (in_array($current_tab, array('mysummary', 'mybyresponse', 'myvall'))) {
            $inactive[] = 'myreport';
            $activated[] = 'myreport';
            $row2 = array();
            ...
            if ($questionnaire->capabilities->downloadresponses) {
                $argstr2 = $argstr.'&action=dcsv'.'&sid='.$questionnaire->sid;
                $row2[] = new tabobject('downloadcsv', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
    ...
    if ($questionnaire->capabilities->readallresponseanytime && $numresp > 0 && $owner && $numselectedresps > 0) {
        ...
        if (in_array($current_tab, array('vall', 'vresp', 'vallsummary', 'deleteall', 'downloadcsv',
            ...
            $argstr2 = $argstr.'&byresponse=1&action=notanswered';
            $row2[] = new tabobject('notanswered', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
                                    get_string('notanswered', 'questionnaire'));

        }

        if (in_array($current_tab, array('vallsummary', 'deleteall', 'downloadcsv'))) {
            $inactive[] = 'vall';
            $activated[] = 'vall';
            $row3 = array();
            $argstr2 = $argstr.'&action=vall';
            $row3[] = new tabobject('vallsummary', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
                                    get_string('summary', 'questionnaire'));
            ...
            if ($questionnaire->capabilities->downloadresponses) {
                $argstr2 = $argstr.'&action=dcsv';
                $row3[] = new tabobject('downloadcsv', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
                                        get_string('downloadtext'));
            }
        }
        ...
        if (in_array($current_tab, array('vallsummary', 'deleteall', 'downloadcsv'))) {
            ...
            $row2 = array();
            $argstr2 = $argstr.'&action=vall';
            $row2[] = new tabobject('vallsummary', $CFG->wwwroot.htmlspecialchars('/mod/questionnaire/report.php?'.$argstr2),
                                    get_string('summary', 'questionnaire'));
