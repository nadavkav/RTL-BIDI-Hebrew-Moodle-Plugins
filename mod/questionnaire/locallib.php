<?php // $Id: locallib.php,v 1.49.2.28 2008/06/24 15:10:15 mchurch Exp $
...
/// TODO - Need to rework this. Too much crossover with ->view method.
 ...
 // JR save each section 's $formdata somewhere in case user returns to that page when navigating the questionnaire...
    ...
    function print_survey_start($message, $section, $num_sections, $has_required, $rid='') {
        ...
        if ($section == 1) {
            if ($this->survey->subtitle) {
                echo '<h4 class="surveySubtitle">'.s($this->survey->subtitle).'</h4>';
            }
            if ($this->survey->info) {
    ...
    function survey_print_render($message = '', $referer='', $courseid) {
        ...
        if ($this->resume && !isset($formdata) && empty($formdata->rid) ) {
            $formdata->rid = $this->get_response($USER->id, $formdata->rid);
        }

        if (!empty($formdata->rid)) {
        // If we're viewing a response, use this method.
            $this->view_response($formdata->rid);
            return;
        }
 ...
 */
    function response_select_name($rid, $qids = null) {
        global $CFG;
        $res = $this->response_select($rid, 'position,type_id,name', $csvexport = true);
        $nam = array();
        reset($res);
        $subqnum = 0;
        $oldpos = '';
...
//        $mail->From     = 'validemailaddress';
    ...
    function response_select($rid, $col = null, $csvexport = false) {
        global $CFG;
                    ...
                    if ($key != 'content') { // no need to keep question text - ony keep choice text and rank
                        if ($key == 'ccontent') {
                            if ($osgood) {
                                ...
                                $val = html_to_text($contentleft.'|'.$contentright);
                            } else {
                                ...
                                if ($contents->modname) {
                                    ...
                                    $val = html_to_text($contents->text);
                                    $val = ereg_replace("[\r\n\t]", ' ', $val);
                                }
                            }
                        }
...
//            $title = userdate($ridssub[$curr_pos]);
    ...
    /* {{{ proto string survey_results(int survey_id, int precision, bool show_totals, int question_id, array choice_ids, int response_id)
        ...
        string. */
    function survey_results($precision = 1, $showTotals = 1, $qid = '', $cids = '', $rid = '', $guicross='', $uid=false, $groupid='') {
        global $CFG, $SESSION;
    ...
    $question->display_results($rids, $guicross);
...
// 			}
    ...
    function export_csv($filename) {
        $umask = umask(0077);
        $fh = fopen($filename.'.csv', 'w'); //(nadavkav)
        umask($umask);
        if(!$fh)
            return 0;
    ...
    function check_date ($thisdate, $insert=false) {
        ...
        if (preg_match('/(%[mdyY])(.+)(%[mdyY])(.+)(%[mdyY])/', $dateformat, $matches)) {
            ...
            if (count($date_pieces) > 1) {
                ...
                $day = max(1, $day);
                if (!$thisdate = mktime(0, 0, 0, $month, $day, $year)) {
                    return 'wrongdaterange';
                } else {
                    if ($insert) {
                        $thisdate = trim(userdate ($thisdate, '%Y-%m-%d', '', false));
                    } else {
                        $thisdate = trim(userdate ($thisdate, $dateformat, '', false));
                    }
                }
                return $thisdate;
            }
