<?php // $Id: questiontypes.class.php,v 1.31.2.21 2008/06/20 13:36:54 mchurch Exp $
...
//            $this->numberalias = $question->numberalias;
    ...
    function get_response_rank_results($rids=false) {
        ...
        if($this->type_id  == 8) { //Rank
         // JR there can't be an !other field in rating questions ???
            ...
            $sql = 'SELECT C.id, C.content, AVG(A.rank+1) AS average '.
...
/// Display Methods
    ...
    function display_resp_multiple_results($rids=false, $guicross=false) {
	    ...
	    //////////////////////////////////////////////////////////////////////////////////////
	    // pre-calculate means (for standard deviation later use)
	    $subtotal = 0;
	    $elements = 0;
	    $mean = 0;
	    $rowid=1;
	    foreach ($rows as $idx => $row) {
	      //echo "[count=".$row->num." id=".$rowid. "]";
	      $subtotal += ($row->num * $rowid++);
	      $elements += $row->num;
	    }
	    $mean = $subtotal / $elements;
	    //echo " mean=".$mean . " elements=".$elements ."<br>";

	    // calculate STD - standard deviation (as defined by http://en.wikipedia.org/wiki/Standard_deviation)
	    $subtotalstd = 0;
	    $rowid=1;
	    foreach ($rows as $idx => $row) {
	      $subtotalstd += ($row->num  * ($rowid - $mean)*($rowid - $mean));
	      //echo "std element=".($row->num  * ($rowid - $mean)*($rowid - $mean))."<br>";
	      $rowid++;
	    }
	    $std = sqrt($subtotalstd / $elements);
	    //echo "std=". $std;
	    //////////////////////////////////////////////////////////////////////////////////////
	    ...
	    echo "<div><strong>".get_string('multiplestd', 'questionnaire')." : ".sprintf("%0.2f",$std)."</strong></div>";
        } else {
            print_string('noresponsedata', 'questionnaire');
        }
    }
...
//echo "<hr>debug<hr> rids=".count($rids);
...
//echo "<hr>found (n) choice rows = ".count($rows)."<hr>";      	
		foreach ($rows as $row)
		{
		  ...
		  //echo "choice text = ".$row->content."<br/>";
//echo "<hr>"."question_id=".$this->id." AND choice_id=".$row->id .$ridstr."<br/>";
		  $values = get_records_select('questionnaire_response_rank', 'question_id='.$this->id.' AND choice_id='.$row->id . $ridstr );
//echo "found X values = ".count($values)."<br/>";
		  //calculate means (for standard deviation later use)
		  $subtotal = 0;
		  $elements = 0;
		  $mean = 0;
		  ...
		  foreach ($values as $value){
                         $rank = $value->rank + 1;
		    $subtotalstd +=  ($rank - $mean)*($rank - $mean);
		  }
		  $std = sqrt($subtotalstd / $elements);
		  //echo $row->content." std=". $std ."<hr>";
		  $this->counts[$row->content]->std = $std;
		}
    ...
    function rate_survey_display($data) { // Rate
        ...
        if ( ($num != $nbchoices) && ($num!=0) ) {
            questionnaire_notify(get_string('checkallradiobuttons', 'questionnaire'));
        }
        $bgr = 'qntype r0';
        foreach ($this->choices as $cid => $choice) {
            if (isset($choice->content)) {
                ...
                for ($j = 0; $j < $this->length; $j++) {
                    ...
                    echo '<td style="width:40px; text-align:center" class="'.$bg.'">';
                ...
                if ($osgood) {
                    ...
                    echo '<td class="'.$bgr2.'">&nbsp;'.$contentright.'</td>';
                }
                echo '</tr>';
            }
        }
            ...
            if (strpos($choice->content, '!other') !== 0) {
                ...
                if ($id == $checked) {
                    echo '<span class="selected">'.
                         ...
                         ($choice->content === '' ? $id : $choice->content).'</span><br />';
                } else {
                    echo '<span class="unselected">'.
...
///****
    ...
    function mkrespercent($total, $precision, $showTotals, $guicross=false) {
        ...
        if (!empty($this->counts) && is_array($this->counts)) {
            ...
            while(list($content,$num) = each($this->counts)) {
                ...
                $tabledata = array_merge($tabledata, array($content, $out, $num));
    ...
    function mkreslistdate($total, $precision, $showTotals) {
        ...
        if (!empty($this->counts) && is_array($this->counts)) {
            ...
            while(list($text,$num) = each($this->counts)) {
                $text = userdate ( $text, $dateformat);    // change timestamp into readable dates
                $table->data[] = array($num, $text);
            }
        } else {
            $table->data[] = array('', get_string('noresponsedata', 'questionnaire'));
