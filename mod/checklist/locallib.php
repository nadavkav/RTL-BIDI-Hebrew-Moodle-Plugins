...
class checklist_class {
    ...
    function view_progressbar() {
        ...
        if ($totalitems > $requireditems) {
            echo '<div style="percentcomplete">';
        ...
        echo '<div class="percentcompleteall">';
                    ...
                    if ($useritem && ($useritem->position == $item->position)) {
                        ...
                        while ($useritem && ($useritem->position == $item->position)) {
                            ...
                            if (isset($useritem->editme)) {
                                ...
                                $note = $itemtext[1];
                                echo '<li>';
                                echo '<div style="float: '.$alignleft.';">';
                                if ($showcheckbox) {
        ...
        if ($showbars) {
            if ($ausers) {
                ...
                foreach ($ausers as $auser) {
                    ...
                    echo '<div style="float: '.$alignleft.'; width: 30%; text-align: '.$alignright.'; margin-right: 8px; ">'.$userlink.$vslink.'</div>';
                    
                    echo '<div class="checklist_progress_outer">';
                    echo '<div class="checklist_progress_inner" style="width:'.$percentcomplete.'%; background-image: url('.$CFG->wwwroot.'/mod/checklist/images/progress.gif);" >&nbsp;</div>';
                    echo '</div>';
                    echo '<div style="float:'.$alignleft.'; width: 3em;">&nbsp;'.sprintf('%0d%%',$percentcomplete).'</div>';
                    echo '<div style="float:'.$alignleft.';">&nbsp;('.$tickeditems.'/'.$totalitems.')</div>';
                    echo '<br style="clear:both;" />';
                }
                echo '</div>';
            }
        ...
        foreach ($table->data as $key => $row) {
            ...
            foreach ($row as $key => $item) {
                ...
                if ($key == 0) {
                    ...
                    $output .= '<td style=" text-align: '.$alignleft.'; width: '.$table->size[0].';" class="cell c0">'.$item.'</td>';
                } else {
                    $size = $table->size[$key];
                    $img = '&nbsp;';
