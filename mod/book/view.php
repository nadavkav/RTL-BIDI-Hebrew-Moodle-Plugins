<?PHP // $Id: view.php,v 1.4.2.2 2008/08/13 23:22:32 skodak Exp $
...
$clean      = optional_param('clean', -1, PARAM_BOOL);    // Edit mode
...
$buttons = $allowedit ? '<table cellspacing="0" cellpadding="0"><tr><td>'.update_module_button($cm->id, $course->id, $strbook).'</td>'.
           ...
           : '&nbsp;';
    // enable a CLEAN - bare book display with no header and footer from the theme (useful for iframe) (nadavkav)
    if ($clean=='1') {
        print_header();
        $edit=0;
    } else {
...
$chnavigation = '';
if (right_to_left()) {
  $imgnavprev = "pix/nav_next.gif";
  $imgnavprevdis = "pix/nav_prev_dis_rtl.gif";
  $imgnavnext = "pix/nav_prev.gif";
  $imgnavexit = "pix/nav_exit_rtl.gif";

} else {
  $imgnavprev = "pix/nav_prev.gif";
  $imgnavprevdis = "pix/nav_prev_dis.gif";
  $imgnavnext = "pix/nav_next.gif";
  $imgnavexit = "pix/nav_exit.gif";

}

if ($previd) {
    $chnavigation .= '<a title="'.get_string('navprev', 'book').'" href="view.php?id='.$cm->id.'&amp;chapterid='.$previd.'"><img src="'.$imgnavprev.'" class="bigicon" alt="'.get_string('navprev', 'book').'"/></a>';
} else {
    $chnavigation .= '<img src="'.$imgnavprevdis.'" class="bigicon" alt="" />';
}
if ($nextid) {
    $chnavigation .= '<a title="'.get_string('navnext', 'book').'" href="view.php?id='.$cm->id.'&amp;chapterid='.$nextid.'"><img src="'.$imgnavnext.'" class="bigicon" alt="'.get_string('navnext', 'book').'" /></a>';
} else {
    $sec = '';
    if ($section = get_record('course_sections', 'id', $cm->section)) {
        $sec = $section->section;
    }
    $chnavigation .= '<a title="'.get_string('navexit', 'book').'" href="../../course/view.php?id='.$course->id.'#section-'.$sec.'"><img src="'.$imgnavexit.'" class="bigicon" alt="'.get_string('navexit', 'book').'" /></a>';
}
...
// make sure toc is generating clean links too (nadavkav)
if ($clean=='1') {
    $editlink = '&clean=1';
} else {
    $editlink = '';
