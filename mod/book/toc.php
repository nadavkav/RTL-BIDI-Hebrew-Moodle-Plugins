...
if ($print) { ///TOC for printing
    ...
    foreach($chapters as $ch) {
        ...
        if ($ch->id == $chapter->id) {
            ...
            $toc .= '<a title="'.s($title).'" href="view.php?id='.$cm->id.'&amp;chapterid='.$ch->id.$editlink.'">'.$title.'</a>';
        }
        $toc .=  '&nbsp;&nbsp;';
        if ($i != 1) {
    ...
    foreach($chapters as $ch) {
        ...
        if (!$ch->hidden) {
            ...
            if ($ch->id == $chapter->id) {
                ...
                $toc .= '<a title="'.s($title).'" href="view.php?id='.$cm->id.'&amp;chapterid='.$ch->id.$editlink.'">'.$title.'</a>'; // (nadavkav)
            }
            $toc .= (!$ch->subchapter) ? '<ul>' : '</li>';
            $first = 0;
        }
