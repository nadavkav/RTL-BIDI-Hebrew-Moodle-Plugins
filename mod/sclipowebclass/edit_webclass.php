<?php // $Id: edit_webclass.php,v 1.4 2009/09/23 14:49:51 alexsclipo Exp $
...
if (isset($_POST['submitted'])) {
	...
	sclipo_updateWebClass($_SESSION["sclipo_id"], $USER->username, $webclass);

	$updatedClass = new SclipoWebClass();
	$updatedClass->name = $_POST["title"];
	$updatedClass->id = $form->id;
	sclipowebclass_update_instance($updatedClass);
	rebuild_course_cache($course->id);	// Need to rebuild Moodle cache
    ...
    $event->instance=$mod->instance;
    $event->timestart=make_timestamp($year, $month, $day, $hour, $min, $tz);
    $event->timeduration=0;
    $event->eventtype="open";
    $event->visible=1;

	sclipowebclass_update_event($event);	// So calendar can find this web class

    // course_modules and course_sections each contain a reference
    // to each other, so we have to update one of them twice.

	redirect("$CFG->wwwroot/mod/$mod->modulename/view.php?id=$mod->coursemodule", "Redirecting, please wait ...", 0);
    ...
    if (!empty($update) and confirm_sesskey()) {
		...
		$webclass = sclipo_getWebClassInfo($_SESSION["sclipo_id"], $USER->username, $form->reference);
...
function validateForm(form)
{
	...
	var dateFormat = form.start_date.value.split("/");
		...
		if (form.max_students.value == '' || isNaN(form.max_students.value) || form.max_students.value>100) {
...
<a name="top"></a>
<form method="post" class="standard" action="<?php echo $CFG->wwwroot.'/mod/sclipowebclass/edit_webclass.php'; ?>" id="create-webclass" onsubmit="return validateForm(this);">
			...
			<input type="hidden" name="ref"          value="<?php  echo $webclass["reference"]; ?>" />
            ...
            <div id="main-extended">
                ...
                <div>
                    ...
                    <fieldset>
                        ...
                        <label for="title">נושא המפגש <em>*</em></label>
                        <p style="margin-bottom: 0px ! important; padding-top: 5px;">כותרת ברורה יכולה לסייע לתלמידים להבין על מה המפגש.</p>
                        <input style="background-color: #FFFFFF"; type="text" value="<?php echo $webclass["title"]; ?>" class="required" id="title" name="title"/>
                                                                        <label for="description">תאור המפגש (מספר תווים מירבי : 1000) <em>*</em></label>
                        <p style="margin-bottom: 0px ! important; padding-top: 5px;">הקפידו על תאור קצר וברור.</p>
                        <textarea style="background-color: #FFFFFF"; class="expanding" cols="40" id="description" name="description" style="overflow: hidden; min-height: 80px; display: block;"><?php echo $webclass["description"]; ?></textarea>

						<label for="tags">תוויות <em>*</em> (הזינו 2 תוויות לפחות. מופרדות בתו רווח)</label>
						<p style="margin-bottom: 0px ! important; padding-top: 5px;">תוויות מסייעות לתלמידים למצוא את המפגש.</p>
                        <input style="background-color: #FFFFFF"; type="text" value="<?php echo $webclass["tags"]; ?>" class="required {maxlength: 255}" id="tags" name="tags" maxlength="255"/>


                		<p class="clear"/>
                        <input type="text" value="1" style="display: none;" class="required" id="confirm_your_tz" name="confirm_your_tz"/>


						<label for="method_face_date">תאריוך וזמן תחילת המפגש<em>*</em></label>
						...
						<div id="change_my_time_zone" style="background-color: #D4D4D4; display: none; margin-top: 10px; padding: 10px;">
							<h2>אזון זמן <a href="#" class="back" style="float: right;" onclick="$(document).trigger('close.facebox'); return false;"> close</a></h2>
							...
							<select name="tzones" class="tzones">
								...
								<option value="Asia/Jerusalem" <?php if ($timezone == "Asia/Jerusalem") echo "SELECTED"; ?>>ירושלים - [GMT +2]</option>
						...
						<input style="background-color: #FFFFFF";" readonly="readonly" type="text" id="start_date" class="date required" name="method_face_date" value="<?php echo $webclass["class_date"]; ?>" />    starts at

						<span class="timeEntry_wrap"><input value="<?php echo $webclass["time"]; ?>" style="vertical-align: baseline; width: 45px; background-color: #FFFFFF;" size="5" name="timeEntry" id="timeEntry"/></span>
						<span class="subtitle" id="schedule-now"> <a class="wc_start_now" href="#" onclick="scheduleNow();">התחילו את המפגש עכשיו!</a></span>
						<label for="duration">משך המפגש: <em>*</em></label>
                        <input style="background-color: #FFFFFF"; type="text" value="<?php echo $webclass["duration"]; ?>" class="number required {min:5,max: 300}" id="duration" name="duration"/> minutes

						 <label for="max_students">מספר תלמידים (מספר מירבי 100) <em>*</em></label>
                        <p style="margin-bottom: 0px ! important; padding-top: 5px;">Indicate how many students can attend this live web class..</p>
                        <input style="background-color: #FFFFFF"; type="text" value="<?php echo $webclass["max_students"]; ?>" class="number required {max: 100}" id="max_students" name="max_students"/>


						<br /><br />
						<input id="submit" type="submit" value="שמרו שינויים" />
						</fieldset>
						<div style="padding-left: 100px;">
						<img src="<?php echo $scimg;?>orange-info-icon.png" />
						<span style="font-weight: bold;">הצגת מסמכים בזמן המפגש</span>
						<br />
...
You can add your documents after creating this web class.
			...
			<div style="background-color: #D4D4D4; width: 200px; padding: 10px;">
						<input type="checkbox" name="publicwebclass" value="public" style="font-weight: bold;" <?php if ($webclass["public"]) echo "checked"; ?>><span style="font-weight: bold;"> Make Web Web Class Public</span></input><br /><br />
