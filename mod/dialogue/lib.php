<?php // $Id: lib.php,v 1.6 2009/03/25 23:20:45 deeknow Exp $
...
//////////////////////////////////////////////////////////////////////////////////////
function dialogue_get_conversations_unique_subject($dialogue, $user, $condition = '', $order = '') {
    global $CFG, $COURSE;
...
//////////////////////////////////////////////////////////////////////////////////////
function dialogue_get_conversations_by_user($user, $condition = '', $order = '') {
    global $CFG, $COURSE;
...
/*    if (! $cm = get_coursemodule_from_instance("dialogue", $dialogue->id, $COURSE->id)) {
  ...
  */  
    ...
    if (has_capability('mod/dialogue:viewall', get_context_instance(CONTEXT_MODULE, $cm->id))) {
    	...
    	$whereuser = ' (c.userid = '.$user->id.' OR c.recipientid = '.$user->id.') ';
    }
    
    $sql  = "SELECT c.*, d.course, cr.shortname , COUNT(e.id) AS total, COUNT(r.id) as readings ";
    $sql .= "FROM {$CFG->prefix}dialogue_conversations c ";
    $sql .= "LEFT JOIN {$CFG->prefix}dialogue d ON d.id = c.dialogueid ";
    $sql .= "LEFT JOIN {$CFG->prefix}course cr ON cr.id = d.course ";
    $sql .= "LEFT JOIN {$CFG->prefix}dialogue_entries e ON e.conversationid = c.id ";
    $sql .= "LEFT JOIN {$CFG->prefix}dialogue_read r ON r.entryid = e.id AND r.userid = $user->id ";
    $sql .= "WHERE $whereuser $condition ";
    $sql .= "GROUP BY c.id, c.userid, c.dialogueid, c.recipientid, c.lastid, c.lastrecipientid, c.timemodified, c.closed, c.seenon, c.ctype, c.format, c.subject, c.groupid, c.grouping ";
    $sql .= "ORDER BY $order ";
