...
************************************************/
    ...
    elseif ($params->action == 'openconversation' ) {
		...
		foreach ($params->recipientid as $selectedrecipientkey => $selectedrecipientid) {
		  $recipients[] = get_record("user", "id", $selectedrecipientid);
		}
                $groupid = 0;
                $grouping = 0;
            }
    ...
    /****************** delete conversation (permission for teachers only ???) ********************/
    elseif ($params->action == 'deleteconversation') {

        print_simple_box( text_to_html($dialogue->intro) , "center");
        echo "<br />";

        dialogue_delete_conversation($dialogue->id,$conversation->id) ;
	 redirect("view.php?id=$cm->id&amp;pane=1", get_string("conversationdeleted", "dialogue"),5);
    }
