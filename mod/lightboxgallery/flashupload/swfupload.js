...
SWFUpload.prototype.appendFlash = function () {
	...
	targetElement.appendChild(container);
       //hgmod:start
	isFalseFlashVer = false;
	FlashVer = this.getFlashVersion();
	if(FlashVar=null) {
	    isFalseFlashVer = true;
	} 	
	versionMajor = parseInt(FlashVer[0]) || 0;
	versionMinor = parseInt(FlashVer[1]) || 0;		
	versionRevision = parseInt(FlashVer[2]) || 0; 
	...
	if( versionMajor < 9 ) {									
	    ...
	    if( versionMajor == 9 ){
		if( versionMinor  < 1 && versionRevision < 60 ) {
		    isFalseFlashVer = true;
		}
	    }
	}	
       ...
       if(isFalseFlashVer) {
	    document.getElementById("flashUI").innerHTML = '<span class="flasherror"><img class="flashicon" src="' + strflashpix + '" alt="Adobe Flash" />&nbsp;' + strflashupload + ':&nbsp;<a  href="http://www.adobe.com/products/flashplayer" target="_blank" class="super">Flashplayer Download&nbsp;(' + strneeded + ')</a></span>';
	}
	else {
...
//hgmod:start
SWFUpload.prototype.getFlashVersion = function () {
	arrVersion = [];										
	if(navigator.plugins && navigator.mimeTypes.length){
		var x = navigator.plugins["Shockwave Flash"];
		if(x && x.description) {
			arrVersion = x.description.replace(/([a-z]|[A-Z]|\s)+/, "").replace(/(\s+r|\s+b[0-9]+)/, ".").split(".");
		}
