...
if ($form = data_submitted()) {
    ...
    foreach($formarr as $key => $value) {
        if(substr($key,0,7) == 'student' && $value !== '') {
            ...
            $students[$i]->override = 1;
            $students[$i]->statusset = $statlist;
            $students[$i]->remarks = array_key_exists('remarks'.$sid, $formarr) ? $formarr['remarks'.$sid] : '';
            $students[$i]->makeupnotes = array_key_exists('makeupnotes'.$sid, $formarr) ? $formarr['makeupnotes'.$sid] : '';
            $students[$i]->sicknote = array_key_exists('sicknote'.$sid, $formarr) ? $formarr['sicknote'.$sid] : '';
...
$tabhead[] = get_string('makeupnotes','attforblock');
$tabhead[] = get_string('sicknote','attforblock');
...
if ($students) {
    ...
    foreach($students as $student) {
        ...
        //	Print the data of the 'remarks', 'makeupnotes', and 'sicknote' fields

        $table->data[$student->id][] = choose_from_menu($optionlist, 'makeupnotes'.$student->id, ($att ? $att->makeupnotes : ''),'', '', '',  true);
        $table->data[$student->id][] = choose_from_menu($optionlist, 'sicknote'.$student->id, ($att ? $att->sicknote : ''),'', '', '',  true);
