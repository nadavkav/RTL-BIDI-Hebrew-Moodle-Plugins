...
//add info to log
add_to_log($course->id, 'attforblock', 'student view', "view.php?course=$course->id&amp;student=$USER->id", $USER->lastname.' '.$USER->firstname);
...
$toprow[] = new tabobject('outstanding', "view.php?id=$id&amp;mode=outstanding{$studstr}",
get_string('outstanding','attforblock'));
$toprow[] = new tabobject('submitted', "view.php?id=$id&amp;mode=submitted{$studstr}",
get_string('submitted','attforblock'));
$toprow[] = new tabobject('cleared', "view.php?id=$id&amp;mode=cleared{$studstr}",
get_string('cleared','attforblock'));
...
if ($mode === 'thiscourse') {
    ...
    if ($mode === 'allcourses') {
        ...
        if ($mode === 'outstanding') {
            print_outstanding_items($user, $cm, $course);
        } else {
            if ($mode === 'submitted') {
                print_submitted_items($user, $cm, $course);
            } else {
                if ($mode === 'cleared') {
                    print_cleared_items($user, $cm, $course);
                }
            }}}}
            print_footer($course);
