...
// Required params
/*$graph_y_data=$_REQUEST['y_data'];
$graph_x_data=$_REQUEST['x_data'];
$graph_title=$_REQUEST['title'];

$graph_y_data=explode(',',$graph_y_data);
$graph_x_data=explode(',',$graph_x_data);*/
//echo "$graphdata";
	 // Create the new graph object
	     
    $roles=required_param('roles',PARAM_RAW);
    $edits=required_param('edits',PARAM_RAW);
    $comments=required_param('comments',PARAM_RAW);
    
    $graph_title = "";
    $graph_x_data = explode(',', $roles);
    $graph_y_data1 = explode(',', $edits);
    $graph_y_data2 = explode(',', $comments);
    ///$graph_y_data1 = array( 7.0, 4.0 );
    ///$graph_y_data2 = array( 1.0, 9.0 );   
    ...
    $graph = new graph(500,250);
     ...
     $graph->parameter['bar_spacing']=100;
	...
	$graph->y_format['bar1'] = array('colour' => 'oumoodleblue', 'bar' => 'fill', 'shadow_offset' => 0.1, 'legend' => 'Edits', 'bar_size' => $graph->parameter['bar_size']);
	$graph->y_format['bar2'] = array('colour' => 'oumoodlegray', 'bar' => 'fill', 'shadow_offset' => 0.1, 'legend' => 'Comments', 'bar_size' => $graph->parameter['bar_size']);
