<?php // $Id: elivenusoap.php,v 1.1.2.3 2009/05/05 18:11:18 jfilip Exp $

/*
$Id: elivenusoap.php,v 1.1.2.3 2009/05/05 18:11:18 jfilip Exp $
...
* @version  $Id: elivenusoap.php,v 1.1.2.3 2009/05/05 18:11:18 jfilip Exp $
* @access   public
*/
class elive_nusoap_base {
	/**
	...
	*/
	...
	'http://www.w3.org/2001/XMLSchema' => array(
		...
		'gYear'=>'','gMonthDay'=>'','gDay'=>'','gMonth'=>'','base64Binary'=>'base64Binary',
		// abstract "any" types
	...
	'http://www.w3.org/2000/10/XMLSchema' => array(
		...
		'timeInstant'=>'string','base64Binary'=>'base64Binary','base64'=>'string','ur-type'=>'array'),
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
		...
		if($type_ns != '' && $type_ns == $this->namespaces['xsd']){
			...
			$type_prefix = 'sas';//ns'.rand(1000,9999);
		...
		*/
        ...
        if($type != '' && isset($this->typemap[$this->XMLSchemaVersion][$type])){
			...
			if ($use == 'literal') {
	        	...
	        	return "<$name$xmlns $atts xsi:type=\"$type_prefix:$type\">$val</$name>";
        	}
        }
					...
					if(is_array($val) && count($val)> 0){
						...
						if(count($array_types) > 1){
							...
							if ($tt_ns != '' && $tt_ns == $this->namespaces['xsd']){
								...
								$tt_prefix = 'sas';//ns' . rand(1000, 9999);
    ...
    */
    ...
    date_default_timezone_set('Asia/Jerusalem'); // (nadavkav patch) support php 5.3
...
* @access   public
*/
function elive_timestamp_to_iso8601($timestamp,$utc=true){
...
* @deprecated
*/
function elive_usleepWindows($usec)
{
	$start = gettimeofday();
...
* @access public
*/
class elive_soap_fault extends elive_nusoap_base {
	/**
	...
	*/
	function elive_soap_fault($faultcode,$faultactor='',$faultstring='',$faultdetail=''){
		parent::nusoap_base();
...
* @access   public
*/
class elive_XMLSchema extends elive_nusoap_base  {
	...
	* @access   public
	*/
	function elive_XMLSchema($schema='',$xml='',$namespaces=array()){
		parent::nusoap_base();
...
* @access   public
*/
class elive_soapval extends elive_nusoap_base {
	/**
	...
	* @access   public
	*/
  	function elive_soapval($name='soapval',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false) {
		parent::nusoap_base();
		$this->name = $name;
		$this->type = $type;
		$this->value = $value;
...
* @access public
*/
class elive_soap_transport_http extends elive_nusoap_base {
...
//curl_setopt($this->ch, CURLOPT_CAINFO, 'f:\php-4.3.2-win32\extensions\curl-ca-bundle.crt');		
	...
	*/
	function setEncoding($enc='gzip, deflate') {
      ...
      if (PHP_VERSION < 6) { set_magic_quotes_runtime($magic_quotes); } // nadavkav patch php 5.3 MDL-20877
...
* @access   public
*/
class elive_soap_server extends elive_nusoap_base {
	/**
	...
	* @access   public
	*/
	function elive_soap_server($wsdl=false){
		parent::nusoap_base();
		// turn on debugging?
		global $debug;
		...
		if (!function_exists('call_user_func_array')) {
			if ($class == '') {
				...
				if ($delim == '..') {
					...
					$instname = "\$inst_".time();
					$funcCall = $instname." = new elive_".$class."(); ";
	...
	*/
    function parseRequest($headers, $data) {
		...
		$parser = new elive_soap_parser($data,$this->xml_encoding,'',$this->decode_utf8);
		// parser debug
	...
	*/
	function fault($faultcode,$faultstring,$faultactor='',$faultdetail=''){
		...
		$this->fault = new elive_soap_fault($faultcode,$faultactor,$faultstring,$faultdetail);
    ...
    */
    function configureWSDL($serviceName,$namespace = false,$endpoint = false,$style='rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
        ...
        $this->wsdl->schemas[$schemaTargetNamespace][0] = new elive_xmlschema('', '', $this->wsdl->namespaces);
...
* @access public 
*/
class elive_wsdl extends elive_nusoap_base {
    ...
    function parseWSDL($wsdl = '')
    {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
        	...
        	// get wsdl
	        $tr = new elive_soap_transport_http($wsdl);
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->status = 'schema';
            $this->currentSchema = new elive_xmlschema('', '', $this->namespaces);
            $this->currentSchema->schemaStartElement($parser, $name, $attrs);
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
        } else {
...
* @access   public
*/
class elive_soap_parser extends elive_nusoap_base {
	...
	* @access   public
	*/
	function elive_soap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		parent::nusoap_base();
		$this->xml = $xml;
...
* @access   public
*/
class elive_soap_client extends elive_nusoap_base  {
	...
	* @access   public
	*/
	function elive_soap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		parent::nusoap_base();
	...
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if($namespace != '' && !isset($this->wsdl->namespaces[$namespace])){
				$nsPrefix = 'sas';//ns' . rand(1000, 9999);
		...
		switch(true){
			...
			case ereg('^http',$this->endpoint):
				...
				if($this->persistentConnection == true && is_object($this->persistentConnection)){
					...
					$http = new elive_soap_transport_http($this->endpoint);
					if ($this->persistentConnection) {
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating soap_parser');
		$parser = new elive_soap_parser($data,$this->xml_encoding,$this->operation,$this->decode_utf8);
	...
	*/
	function getProxy(){
		...
		eval("\$proxy = new elive_soap_proxy_$r('');");
	...
	*/
	...
	";
		...
		$evalStr = 'class elive_soap_proxy_'.$r.' extends elive_soap_client {
	'.$evalStr.'
}';
		return $evalStr;
	}
