 ...
 */
function imagedit_upload_response($fileid, $course, $attemptid, $questionid, $maxbytes) {
    global $CFG;

    require_once($CFG->dirroot.'/lib/uploadlib.php');
    $um = new upload_manager($fileid,true,false,$course,true,$maxbytes,true);
    if ($um->preprocess_files()) {
        $dir = imagedit_quiz_file_area_name($attemptid, $questionid);
        if(!imagedit_quiz_file_area($dir)) {
    ...
    return get_string('uploaderror', 'qtype_imagedit');
}
 ...
 */
function imagedit_quiz_file_area_name($attemptid, $questionid) {
    global $CFG, $USER;
 ...
 * @return string path to file area
 */
function imagedit_quiz_file_area($dir) {
 ...
 */
function imagedit_get_student_answer($attemptid, $questionid){
    global $CFG;

    $filearea = imagedit_quiz_file_area_name($attemptid, $questionid);

    $output = '';

    if ($basedir = imagedit_quiz_file_area($filearea)) {
