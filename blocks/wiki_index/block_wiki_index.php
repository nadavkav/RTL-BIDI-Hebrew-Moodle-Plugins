...
/**
 * This file contains the wiki index class for moodle blocks.
 *
 * @author DFWiki LABS
 ...
 * @version  $Id: block_wiki_index.php,v 1.20 2008/08/25 10:46:16 pigui Exp $
 * @license http://www.gnu.org/copyleft/gpl.html GNU Public License
 * @package Wiki_Blocks
 */

class block_wiki_index extends block_base {
    ...
    function init() {
        ...
        $this->title = get_string('block_index', 'wiki').helpbutton ('index', get_string('block_index', 'wiki'), 'wiki', true, false, '', true);
	...
	function built_index($page,$level){
		...
		if (!in_array($page,$this->pagesel)){
			...
			if (!$pageinfo = wiki_page_last_version ($page)){
				//empty page
				//put in result
				return '<li class="wiki_listme">'.$this->images['square'].format_text($this->trim_string($page,25),FORMAT_PLAIN).'<a href="'.$dir.'&amp;page='.urlencode($page).'">?</a></li>';
			} else {
				...
				//put in result
				if (count($links)==0){
					//$res = '<li class="wiki_listme">'.$this->images['square'].'<a href="view.php?id='.$WS->cm->id.'&amp;gid='.$groupmember->groupid.'&amp;page='.$page.'">'.$page.'</a>'.'</li>';
					$res = '<li class="wiki_listme">'.$this->images['square'].'<a href="'.$dir.'&amp;page='.urlencode($page).'" title="'.urlencode($page).'">'.format_text($this->trim_string($page,20),FORMAT_PLAIN).'</a>'.'</li>';
				}else{
                    ...
                    $res = '<li class="wiki_listme">'.$image_ico.'<a href="'.$dir.'&amp;page='.urlencode($page).'" title="'.urlencode($page).'">'.format_text($this->trim_string($page,20),FORMAT_PLAIN).'</a>
			...
			return '<li class="wiki_listme">'.$this->images['square'].'<a href="'.$dir.'&amp;page='.urlencode($page).'" title="'.urlencode($page).'"><i>'.format_text($this->trim_string($page,20),FORMAT_PLAIN).'</i></a>'.'</li>';
		}
	}
