  ...
  */
		...
		if ($xml = $mform->get_file_content('userfile')) {
			...
			if(isset($data['report']['@']['version'])){
				...
				foreach($data['report']['#'] as $key=>$val){
					...
					$newreport->{$key} = addslashes(trim($val[0]['#']));
				}
				$newreport->courseid = $course->id;
				$newreport->ownerid = $USER->id;
				if(!insert_record('block_configurable_reports_report',$newreport))
	...
	if($reports){
		...
		foreach($reports as $r){
			...
			else if(! $coursename = get_field('course','fullname','id',$r->courseid))
				$coursename = get_string('deleted');
			else
				$coursename = '<a href="'.$CFG->wwwroot.'/course/view.php?id='.$r->courseid.'">'.$coursename.'</a>';
				
			if($owneruser = get_record('user','id',$r->ownerid))
				$owner = '<a href="'.$CFG->wwwroot.'/user/view.php?id='.$r->ownerid.'">'.fullname($owneruser).'</a>';
			else
				$owner = get_string('deleted');
			
			$editcell = '';
			$editcell .= '<a title="'.$stredit.'"  href="editreport.php?id='.$r->id.'"><img src="'.$CFG->pixpath.'/t/edit.gif" class="iconsmall" alt="'.$stredit.'" /></a>&nbsp;&nbsp;';
			$editcell .= '<a title="'.$strdelete.'"  href="editreport.php?id='.$r->id.'&amp;delete=1&amp;sesskey='.$USER->sesskey.'"><img src="'.$CFG->pixpath.'/t/delete.gif" class="iconsmall" alt="'.$strdelete.'" /></a>&nbsp;&nbsp;';
			
			
			if (!empty($r->visible)) {
				$editcell .= '<a title="'.$strhide.'" href="editreport.php?id='.$r->id.'&amp;hide=1&amp;sesskey='.$USER->sesskey.'">'.'<img src="'.$CFG->pixpath.'/t/hide.gif" class="iconsmall" alt="'.$strhide.'" /></a> ';}
			else {
				$editcell .= '<a title="'.$strshow.'" href="editreport.php?id='.$r->id.'&amp;show=1&amp;sesskey='.$USER->sesskey.'">'.'<img src="'.$CFG->pixpath.'/t/show.gif" class="iconsmall" alt="'.$strshow.'" /></a> ';
			}
			$editcell .= '<a title="'.$strcopy.'" href="editreport.php?id='.$r->id.'&amp;duplicate=1&amp;sesskey='.$USER->sesskey.'"><img src="'.$CFG->pixpath.'/t/copy.gif" class="iconsmall" alt="'.$strcopy.'" /></a>&nbsp;&nbsp;';
			$editcell .= '<a title="'.$strexport.'" href="export.php?id='.$r->id.'&amp;sesskey='.$USER->sesskey.'"><img src="'.$CFG->pixpath.'/i/backup.gif" class="iconsmall" alt="'.$strexport.'" /></a>&nbsp;&nbsp;';
