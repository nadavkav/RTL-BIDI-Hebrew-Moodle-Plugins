...
/** Configurable Reports
  ...
  */
	...
	if(($show || $hide) && confirm_sesskey()){
		$visible = ($show)? 1 : 0;
		if(!set_field('block_configurable_reports_report','visible',$visible,'id',$report->id))
			print_error('cannotupdatereport','block_configurable_reports');
		$action = ($visible)? 'showed' : 'hidden';	
		add_to_log($report->courseid, '', 'report '.$action, '/block/configurable_reports/editreport.php?id='.$report->id, $report->id);	
	...
	if($duplicate && confirm_sesskey()){
		...
		$newreport = $report;
		unset($newreport->id);
		$newreport->name = addslashes(get_string('copyasnoun').' '.$newreport->name);
		$newreport->summary = addslashes($newreport->summary);
		if(! $newreportid = insert_record('block_configurable_reports_report',$newreport))
			print_error('cannotduplicate','block_configurable_reports');
		add_to_log($newreport->courseid, '', 'report duplicated', '/block/configurable_reports/editreport.php?id='.$newreportid, $id);
		header("Location: $CFG->wwwroot/blocks/configurable_reports/managereport.php?courseid=$courseid");
		die;
	}
	...
	if($delete && confirm_sesskey()){
		if(!$confirm){
			...
			$optionsno = array();
			notice_yesno($message, 'editreport.php', 'managereport.php', $optionsyes, $optionsno, 'post', 'get');
			print_footer();
			exit;
		}
		else{
			if(delete_records('block_configurable_reports_report','id',$report->id))
				add_to_log($report->courseid, '', 'report deleted', '/block/configurable_reports/editreport.php?id='.$report->id, $report->id);
			header("Location: $CFG->wwwroot/blocks/configurable_reports/managereport.php?courseid=$courseid");
			die;
		}
	}
	...
	if($editform->is_cancelled()){
		...
		if(empty($report)){
			...
			if(! $lastid = insert_record('block_configurable_reports_report',$data)){
				print_error('errorsavingreport','block_configurable_reports');
			}else{
				add_to_log($courseid, '', 'report created', '/block/configurable_reports/editreport.php?id='.$lastid, $data->name);
			...
			add_to_log($report->courseid, '', 'edit', '/block/configurable_reports/editreport.php?id='.$id, $report->name);
			$reportclass = new $reportclassname($data->id);
			$data->type = $report->type;
			if(! update_record('block_configurable_reports_report',$data)){
				print_error('errorsavingreport','block_configurable_reports');
			}else{
