...
function exabis_get_categories() {
  global $USER;
    ...
    foreach ( $outercategories as $curcategory ) {
      ...
      if($inner_categories) {
        foreach ( $inner_categories as $inner_curcategory ) {
          ...
          $inner_sub_categories = get_records_select("block_exabeporcate", "userid='$USER->id' AND pid='$inner_curcategory->id'", "name asc");
          if($inner_sub_categories) {
            foreach ( $inner_sub_categories as $inner_sub_curcategory ) {
              $categories[$inner_sub_curcategory->id] = format_string($curcategory->name) . $arrow . format_string($inner_curcategory->name) . $arrow . format_string($inner_sub_curcategory->name);
            }
          } else {
            $categories[$inner_curcategory->id] = format_string($curcategory->name) . $arrow . format_string($inner_curcategory->name);
          }
