...
class SharingCart_Restore extends SharingCart_BackupRestoreBase
{
	...
	protected function restoreSection()
	{
		...
		if ($info = restore_read_xml_sections($this->getTempDir().'/moodle.xml')) {
			...
			if (!empty($info->sections) and $section = reset($info->sections)) {
				...
				if ($this->opt_section_status) {
					...
					foreach ($fields as $field) {
						$this->section->$field = $section->$field;
					}
					// ↓で update_record() を呼び出すのでここでは省略
		...
		if (!update_record('course_sections', $this->section))
				...
				foreach ($a as $i => $s) {
					if ($i & 1) {
						...
						if (preg_match('@^\s*<ID>(?:'.implode('|', $no_dup_course_question_ids).')</ID>@', $s)) {
							...
							$this->findLinksAndSetFlags($exclude_flags, $re_file_refs, $s, TRUE);
						} else {
							// 複製された問題       → 除外フラグを消す
							$this->findLinksAndSetFlags($exclude_flags, $re_file_refs, $s, FALSE);
						}
					} else {
						// 偶数インデックス：非マッチ
	...
	protected function findLinksAndSetFlags(&$flags, $res, $s, $set)
	{
		foreach ($res as $re) {
