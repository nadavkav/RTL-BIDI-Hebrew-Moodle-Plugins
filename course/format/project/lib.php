...
/**
* Library of functions for the project course format.
*/
 ...
 */
function project_format_rename_section_links(&$course, $sectionid, $directoryname, $iscopy = true, $ismessage = true) {
    global $CFG;
    $status = '';
    ...
    foreach ($mods as $mod) {
        ...
        if ($mod->section == $sectionid) {
            ...
            if (function_exists($mod->modname.'_get_links') && function_exists($mod->modname.'_rename_links')) {
                ...
                // 注意事項の追加
                if ($ismessage) {
                    notify($mod->modfullname.get_string('notrename','format_project'));
                }
            }
        }
    }
 ...
 */
function project_format_check_directoryname($courseid,$directoryname,$sectionnumber = 0) {
    // コース内のセクション一覧の取得
 ...
 */
function project_is_exist_directory($course,$sectionid)
{
    // ベースディレクトリ名の取得
