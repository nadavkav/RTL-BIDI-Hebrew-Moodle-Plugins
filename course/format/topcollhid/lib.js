...
function topcollhid_init(wwwroot, moodleid, courseid, cookielifetime)
{
    // Init.
    thewwwroot = wwwroot;
    ...
    cssNode.setAttribute('href', "" + thewwwroot + "/course/format/topcollhid/js-override-topcollhid.css");
...
function toggleexacttopic(target,image,toggleNum,reloading)  // Toggle the target tr and change the image which is the a tag within the td of the tr above target
{
    ...
    if((document.getElementById) && ((target != null) && (image !=null)))
    {
        ...
        if (target.style.display == displaySetting)
        {
            ...
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/topcollhid/arrow_down.png)";
            // Save the toggle!
        ...
        else
        {
            ...
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/topcollhid/arrow_up.png)";
            // Save the toggle!
...
function savetopcollhidcookie(value)
{
    // Using Sub cookies, so, name, moodleid/courseid, value.
    if (cookieExpires == null)
    {
        // Session Cookie...
        yuicookie.setSub("mdl_cf_topcollhid",thecookiesubid,value);
    ...
    else
    {
        ...
        yuicookie.setSub("mdl_cf_topcollhid",thecookiesubid,value, { expires: newDate });
        // This is not a Moodle table but in fact the cookies name.
    }
}

// Get the cookie - yum.
function restoretopcollhidcookie()
{
    return yuicookie.getSub("mdl_cf_topcollhid",thecookiesubid); // Returns null if cookie does not exist.
}
...
function reload_toggles()
{
    ...
    var storedval = restoretopcollhidcookie();
    if (storedval != null)
    {
...
{
    savetopcollhidcookie(to36baseString(toggleBinaryGlobal));
}