...
/// Layout the whole page as three big columns.
    echo '<table id="layout-table" class="newsroom" cellspacing="0" summary="'.get_string('layouttable').'"><tr>';
    ...
    while ($section <= $course->numsections) {
        ...
        if ($showsection) {
            ...
            if (!has_capability('moodle/course:viewhiddensections', $context) and !$thissection->visible) {   // Hidden for students
                ...
                print_section_newsroom($course, $thissection, $mods, $modnamesused);
        ...
        foreach ($sectionmods as $modnumber) {
            ...
            if ($mod->modname == "label") {
                ...
                if ($mod->modname == 'oublog') {
					...
					echo '<br/>';
                    /// Get Posts
                    list($posts, $recordcount) = oublog_get_posts($oublog, $oublogcontext, 0, $oublogcm, $currentgroup); //, -1, $oubloguser->id, $tag, $canaudit);
					$i=0;
                    ...
                    //print_r($article );
                    //echo "$article->summary";
					echo '<br/><a '.$linkcss.' '.$extra.        // Title unnecessary!
                     ' href="'.$CFG->wwwroot.'/mod/'.$mod->modname.'/view.php?id='.$mod->id.'">'.
                     '<img src="'.$icon.'" class="activityicon" alt="" /> <span>'.
                     get_string('more','format_newsroom').'</span></a>';
                }
