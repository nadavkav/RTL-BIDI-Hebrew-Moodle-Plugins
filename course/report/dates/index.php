<?php // $Id: index.php,v 1.0 $
//


    require_once('../../../config.php');
    require_once('dateslib.php');
    ...
    $date1       = optional_param('date1', 0, PARAM_FILE); // Date to display - number or some string
 ...
 // Checking the host, whether its a localhost or remote host
...
// Checking user cabability
    require_capability('coursereport/log:view', $context);

// Adding the current action to log, i.e. view course report
    add_to_log($course->id, "course", "report log", "report/dates/index.php?id=$course->id", $course->id);

    $strlogs = get_string('summerydates','report_dates');
	...
	if ($date1) {
            $dateinfo1 = userdate($date1, get_string('strftimedaydate'));
        }
        ...
        switch ($logformat) {
            case 'showashtml':
                ...
                print_heading(format_string($course->fullname) . ": $userinfo");
                print_mnet_log_selector_form1($hostid, $course, $user, $date,$date1, $modname, $modid, $modaction, $group, $showcourses, $showusers, $logformat);

                if($hostid == $CFG->mnet_localhost_id) {
                    print_log1($course, $user, $date,$date1, 'l.time DESC', $page, $perpage,
                            "index.php?id=$course->id&amp;chooselog=1&amp;user=$user&amp;date=$date&amp;date1=$date1&amp;modid=$modid&amp;modaction=$modaction&amp;group=$group",
                            $modname, $modid, $modaction, $group);
                } else {
                    print_mnet_log1($hostid, $id, $user, $date, 'l.time DESC', $page, $perpage, "", $modname, $modid, $modaction, $group);
                }
                break;
            case 'downloadascsv':
                if (!print_log_csv1($course, $user, $date,$date1, 'l.time DESC', $modname,
                ...
                exit;
            case 'downloadasods':
                if (!print_log_ods1($course, $user, $date,$date1, 'l.time DESC', $modname,
                        $modid, $modaction, $group)) {
                    notify("No logs found!");
                    print_footer($course);
                }
            ...
            case 'downloadasexcel':
                if (!print_log_xls1($course, $user, $date,$date1, 'l.time DESC', $modname,
                        $modid, $modaction, $group)) {
                    notify("No logs found!");
                    print_footer($course);
                }
